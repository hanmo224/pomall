        ...
        <div class="row">
            ...
            <div class="box-header with-border">
                <div class="form-group row">
                  ...
                  <div class="col-md-4">
                    <select name="cg_procode" id="firstCategory" class="form-control">
                      <option value="">1차카테고리 선택</option>
                      <c:forEach items="${categoryList }" var="category">
                        <option value="${category.cg_code }">${category.cg_name }</option>
                      </c:forEach>
                    </select>
                    <select name="cg_code" id="secondCategory" class="form-control">
                      <option>2차카테고리 선택</option>
                    </select>
                  </div>
                  <div class="col-md-2">
                   <button type="button"  id="categorySearch" class="btn btn-success">검색</button>
                  </div>
                  <div class="col-md-2">
            ...
            <div class="box-body">
              <table class="table table-bordered">
                ...
                <tr>
                  <td><input type="checkbox" name="check" value="${productVO.pro_code }"></td>
                  <td>
                  	<input type="hidden" name="cg_code" value="${productVO.cg_code }">
                  	<c:out value="${productVO.pro_code }"></c:out>
                  </td>
                  <td>
                    <!-- 역슬래시 \ 가 클라이언트 요청데이타로 사용 불가능.-->
                    <a class="move" href="${productVO.pro_code }"><img src="/admin/product/displayImage?folderName=${productVO.pro_up_folder }&fileName=s_${productVO.pro_img}" /></a>
                    <a class="move" href="${productVO.pro_code }"><c:out value="${productVO.pro_name }" ></c:out></a>
                  </td>
                  <td>
                    <input type="text" name="pro_price" style="width:100px" value="${productVO.pro_price }">
                  </td>
                  <td>${productVO.pro_amount}</td>
                  <td>${productVO.pro_publisher}</td>
                  <td><fmt:formatDate value="${productVO.pro_regdate }" pattern="yyyy-MM-dd HH:mm"/></td>
                  <td>
                  ...
                  <td><button type="button" name="btn_productEdit" data-pro_code="${productVO.pro_code }" class="btn btn-link">수정</button></td>
                  <td><button type="button" name="btn_productDel" data-pro_code="${productVO.pro_code }" class="btn btn-link">삭제</button></td>
                </tr>
                </c:forEach>
  ...
  $(document).ready(function() {
    ...
    let actionForm = $("#actionForm"); // 페이징정보가 있는 form태그. Criteria클래스 필드정보
      ...
      $("a.move").on("click", function(e) {
        ...
        let pro_code = $(this).attr("href");

        let cg_code = $(this).parent().parent().find("input[name='cg_code']").val();
        actionForm.append("<input type='hidden' name='cg_code' value='" + cg_code + "'>");
      ...
      $("button[name='btn_productEdit']").on("click", function() {
        // 수정 상품코드
        actionForm.append("<input type='hidden' name='pro_code' value='" + $(this).data("pro_code") + "'>");
      ...
      $("#btn_checkedModify").on("click", function() {
        ...
        // 자바스크립트 배열문법
        let pro_code_arr = []; // 수정상품코드 배열
        let pro_price_arr = []; //수정상품가격 배열
        let pro_buy_arr = []; // 판매여부 배열

        $("input[name='check']:checked").each(function() {
          pro_code_arr.push($(this).val()); // this는 체크된 것들을 가리킴
        ...
        /*
        console.log("수정상품코드: " + pro_code_arr);
        ...
        */
        ...
        $.ajax({
          ...
          data: {pro_code_arr: pro_code_arr, pro_price_arr:pro_price_arr, pro_buy_arr:pro_buy_arr},
          dataType: 'text',
      ...
      $("#btn_checkedDelete").on("click", function() {
        ...
        $.ajax({
          ...
          type:'post',
          data: {pro_code_arr: pro_code_arr},
          dataType: 'text',
          success: function(result) {
            if(result == "success") {
   ...
   $("#firstCategory").change(function() {
     ...
     $.getJSON(url, function(subCategoryData){
       ...
       for(let i=0; i<subCategoryData.length; i++) {
         optionStr += "<option value='" + subCategoryData[i].cg_code + "'>" + subCategoryData[i].cg_name + "</option>";
       }
   ...
   $("#categorySearch").on("click", function() {
    actionForm.attr("action", "/admin/product/pro_list");
    actionForm.append("<input type='hidden' name='cg_code' value='" + $("#secondCategory").val() + "'>");
